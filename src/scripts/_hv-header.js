...
export class hvHeader {
  ...
  _updateState() {
    ...
    if (shouldFix && !this._fixedEngaged) {
      ...
      // если стартуем скрытыми — входной slide FX не нужен
  ...
  _applyScrollBehavior() {
    switch (this.o.scrollBehavior) {
      ...
      case 'shrink':
        ...
        if (this._dir === 1 && this._acc >= this.o.shrinkShowDelta) {
          this._setCompact(true);
          this._acc = 0;
        } else if (this._dir === -1 && this._acc >= this.o.shrinkHideDelta) {
          this._setCompact(false);
          this._acc = 0;
        }
        this._setHidden(false);
      ...
      case 'reveal':
        ...
        if (this._dir === 1 && this._acc >= this.o.revealHideDelta) {
          this._setHidden(true);
          this._acc = 0;
        } else if (this._dir === -1 && this._acc >= this.o.revealShowDelta) {
          this._setHidden(false);
          this._acc = 0;
        }
        break;
